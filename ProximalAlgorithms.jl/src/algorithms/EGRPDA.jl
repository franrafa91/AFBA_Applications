...
"""
    EGRPDAIteration(; <keyword-arguments>)
...
"""
Base.@kwdef struct EGRPDAIteration{R,Tx,Ty,Tf,Tg,Th,Tl,TL,Tbetaf,Tbetal,Ttheta,Tmu,Tlambda}
    f::Tf = Zero()
    g::Tg = Zero()
    h::Th = Zero()
    l::Tl = IndZero()
...
Base.IteratorSize(::Type{<:EGRPDAIteration}) = Base.IsInfinite()

Base.@kwdef struct EGRPDAState{Tx,Ty,R}
    x::Tx
    y::Ty
    lsearch::R # τₖ, βₖ, ϕₖ
    z::Tx = similar(x)
...
function Base.iterate(iter::EGRPDAIteration, state::EGRPDAState = EGRPDAState(x=copy(iter.x0), y=copy(iter.y0), z=copy(iter.x0), lsearch=[iter.gamma[1],iter.gamma[2]/iter.gamma[1],(1+iter.ψ)/(iter.ψ^2)]))
    if state.x == iter.x0
        costs(iter, state)
    ...
    for i in 0:100
        ...
        if ((τₖ₋₁*σₖ)*norm(state.temp_x)^2
            +2*σₖ*(convex_conjugate(iter.l)(state.ybar)-convex_conjugate(iter.l)(state.y)-dot(state.gradl,state.temp_y))
...
default_stopping_criterion(tol, ::EGRPDAIteration, state::EGRPDAState) = norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf) <= tol
default_solution(::EGRPDAIteration, state::EGRPDAState) = (state.xbar, state.ybar)
default_display(it, ::EGRPDAIteration, state::EGRPDAState) = @printf("%6d | %7.4e\n", it, norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf))

"""
    EGRPDA(; <keyword-arguments>)
...
The returned object has type `IterativeAlgorithm{EGRPDAIteration}`,
...
EGRPDA(;
    ...
    kwargs...
) = IterativeAlgorithm(EGRPDAIteration; maxit, stop, solution, verbose, freq, display, kwargs...)
