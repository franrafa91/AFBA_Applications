...
# Linesearch". In SIAM Journal on Optimization 28 (2018), pp. 411-432.
# DOI: https://doi.org/10.1137/16M1092015.
...
"""
    GPDALIteration(; <keyword-arguments>)
...
If unspecified, functions `g` and `h` default to the identically zero
...
"""
Base.@kwdef struct GPDALIteration{R,Tx,Ty,Tf,Tg,Th,Tl,TL,Tbetaf,Tbetal,Ttheta,Tmu,Tlambda}
    f::Tf = Zero()
    g::Tg = Zero()
    h::Th = Zero()
    l::Tl = IndZero()
...
Base.IteratorSize(::Type{<:GPDALIteration}) = Base.IsInfinite()

Base.@kwdef struct GPDALState{Tx,Ty,R}
    x::Tx
    y::Ty
    lsearch::R # τₖ, βₖ, θₖ
    xbar::Tx = similar(x)
...
function Base.iterate(iter::GPDALIteration, state::GPDALState = GPDALState(x=copy(iter.x0), y=copy(iter.y0),lsearch=[iter.gamma[1],iter.gamma[2]/iter.gamma[1],1.0]))
    if state.x == iter.x0
        costs(iter,state)
        append!(iter.step_list, state.lsearch[1])
        gradient!(state.gradl_new,convex_conjugate(iter.l),state.y)
    end
    ...
    for i in 0:100
        ...
        if ((τₖ*σₖ)*norm(state.temp_x)^2+2*σₖ*(convex_conjugate(iter.l)(state.ybar)-
            convex_conjugate(iter.l)(state.y)-dot(state.gradl,state.temp_y))<=iter.δ*norm(state.temp_y)^2)
            break
        end
        if i == 100
        end
    ...
    state.temp_x .-= state.FPR_x./τₖ₋₁ #FPR_x = -(xₙ-x̄ₙ)
    ...
    state.temp_y .*= θₖ
    state.temp_y .-= state.FPR_y./σₖ  #FPR_x = -(xₙ-x̄ₙ)
    state.temp_y .-= state.gradl
...
default_stopping_criterion(tol, ::GPDALIteration, state::GPDALState) = norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf) <= tol
default_solution(::GPDALIteration, state::GPDALState) = (state.xbar, state.ybar)
default_display(it, ::GPDALIteration, state::GPDALState) = @printf("%6d | %7.4e\n", it, norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf))

"""
    GPDAL(; <keyword-arguments>)
...
where `g` and `h` are possibly nonsmooth and `l` is strongly
convex. Symbol `□` denotes the infimal convolution, and `L` is a linear mapping.

The returned object has type `IterativeAlgorithm{GPDALIteration}`,
...
GPDAL(;
    ...
    kwargs...
) = IterativeAlgorithm(GPDALIteration; maxit, stop, solution, verbose, freq, display, kwargs...)