...
"""
Base.@kwdef struct MAFBAIteration{R,Tx,Ty,Tf,Tg,Th,Tl,TL,Tbetaf,Tbetal,Ttheta,Tmu,Tlambda}
    f::Tf = Zero()
    g::Tg = Zero()
    h::Th = Zero()
    l::Tl = IndZero()
    ...
    gamma::Tuple{R, R} = if lambda != 1
        ...
        T = real(eltype(x0))
        MAFBA_default_stepsizes(L, h, T(theta), T(mu), T(beta_f), T(beta_l))
    end
    τ0::R = eltype(x0)(Inf)
    τᵢ::R = 1e-7
    β₀::R = gamma[2]/gamma[1]
...
Base.IteratorSize(::Type{<:MAFBAIteration}) = Base.IsInfinite()
...
"""
LVuCondatIteration(; kwargs...) = MAFBAIteration(kwargs..., theta=2)
...
"""
MChambollePockIteration(; kwargs...) = MAFBAIteration(kwargs..., theta=2, f=Zero(), l=IndZero())

Base.@kwdef struct MAFBAState{Tx,Ty,array}
    x::Tx
    y::Ty
    x_1::Tx
    smooth::array # [init, Lₖ, τₖ₋₁,θₖ,βₖ,L_normsq]
    # Eventually consider a case where βₖ evolves
    xbar::Tx = similar(x)
    xtilde::Tx = similar(x)
    ybar::Ty = similar(y)
    gradf::Tx = similar(x)
...
function Base.iterate(iter::MAFBAIteration, state::MAFBAState = MAFBAState(x=copy(iter.x0), y=copy(iter.y0),x_1=copy(iter.x0),smooth=[0.,0.,iter.τᵢ, 1., iter.β₀,0.])) # [Lₖ, τₖ₋₁,θₖ₋₁,βₖ]
    if state.x == iter.x0
        append!(iter.x_list,iter.f(state.x)+iter.g(state.x))
        mul!(state.temp_y, iter.L, state.x)
        append!(iter.d_list,iter.h(state.temp_y))
    end
    if state.smooth[1] == 0
        it, Lₖ, τₖ₋₁,θₖ₋₁,βₖ,L_normsq = state.smooth
        mul!(state.x, iter.L', state.y)
        # print("State 1 after 1 step", state.x[1:8], ".\n")
        state.x .+= iter.f(state.x_1) #state.x_1 = x_0
        # print("State 1 after 2 steps", state.x[1:8], ".\n")
        state.x .*= -τₖ₋₁
        state.x .+= state.x_1
        # print("State 0", state.x_1[1:8], ".\n  State 1", state.x[1:8],".\n")
        Lₖ = norm(iter.f(state.x)-iter.f(state.x_1))/norm(state.x-state.x_1)
        state.smooth[1] = 1
        L_normsq=opnorm(iter.L)
        # print("Initial Lk:",Lₖ,"\n")
        τₖ₋₁ = iter.τ0
    else
        it, Lₖ, τₖ₋₁,θₖ₋₁,βₖ,L_normsq = state.smooth
    ...
    # update stepsize
    τₖ = min(1/(2*sqrt(Lₖ^2+(βₖ/(1-iter.c))*L_normsq)),τₖ₋₁*sqrt(1+θₖ₋₁))
    θₖ = τₖ/τₖ₋₁
    state.temp_x .= state.x-state.x_1
    state.temp_x .*= θₖ
    ...
    # No longer needed -- state.temp_x .= iter.theta .* state.xbar .+ (1 - iter.theta) .* state.x
    mul!(state.temp_y, iter.L, state.temp_x)
    state.temp_y .-= state.gradl
    state.temp_y .*= βₖ*τₖ
    state.temp_y .+= state.y
    prox!(state.ybar, convex_conjugate(iter.h), state.temp_y, βₖ*τₖ)

    # perform xbar-update step (CHANGED ORDER OF STATES)
    gradient!(state.gradf, iter.f, state.x)
    state.temp_y .= iter.theta .* state.ybar .+ (1 - iter.theta) .* state.y
    ...
    βₖ = βₖ
    it += 1
    state.smooth .= [it, Lₖ, τₖ₋₁,θₖ₋₁,βₖ,L_normsq]

    print("Iteration ",it,". Stepsize: ",τₖ₋₁,"\n")
    append!(iter.x_list,iter.g(state.x))
    mul!(state.temp_y, iter.L, state.x)
    append!(iter.d_list,iter.h(state.temp_y))
    return state, state
end

default_stopping_criterion(tol, ::MAFBAIteration, state::MAFBAState) = norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf) <= tol
default_solution(::MAFBAIteration, state::MAFBAState) = (state.xbar, state.ybar)
default_display(it, ::MAFBAIteration, state::MAFBAState) = @printf("%6d | %7.4e\n", it, norm(state.FPR_x, Inf) + norm(state.FPR_y, Inf))
...
MAFBA(;
    ...
    kwargs...
) = IterativeAlgorithm(MAFBAIteration; maxit, stop, solution, verbose, freq, display, kwargs...)
...
2. Vũ, "A splitting algorithm for dual monotone inclusions involving cocoercive operators", Advances in Computational Mathematics, vol. 38, no. 3, pp. 667-681 (2013).
"""
MVuCondat(; kwargs...) = MAFBA(; kwargs..., theta=2)
...
1. Chambolle, Pock, "A First-Order Primal-Dual Algorithm for Convex Problems with Applications to Imaging", Journal of Mathematical Imaging and Vision, vol. 40, no. 1, pp. 120-145 (2011).
"""
MChambollePock(; kwargs...) = MAFBA(; kwargs..., f=Zero(), l=IndZero(), theta=2)

function MAFBA_default_stepsizes(L, h::Zero, theta::R, mu::R, beta_f::R, beta_l::R) where R
    return R(1.99) / beta_f, R(1)
end

function MAFBA_default_stepsizes(L, h, theta::R, mu::R, beta_f::R, beta_l::R) where R
